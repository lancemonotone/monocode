 ...
 * WMS REST API
 *
 * @package Wms_Rest_API
 *
 * @author Williams Web Team
 * @version 1.0.0
 */
/*
Plugin Name: WMS REST API
Plugin URI: http://williams.edu
Description: WMS REST API - Requires JSON REST API plugin
Version: 1.0.0
Author: Williams Web Team
...
*/
if(!defined('REST_API_VERSION')) return new WP_Error('wms_rest_api', 'WP REST API not active');
...
class Wms_Rest_API {
	var $debug = true;
	var $namespace = "wms-rest-api";
	var $id = "wms_rest_api";
	var $friendly_name = "WMS REST API";
	var $version = "1.0.0";
	 ...
	 *
	 * @uses add_action()
	 * @uses Wms_Rest_API::wp_register_scripts()
	 * @uses Wms_Rest_API::wp_register_styles()
	 */
	function __construct() {
		...
		$libs = glob( WMS_REST_API_DIRNAME . '/lib/*.php' );
		foreach ( $libs as $lib ) {
			require_once( $lib );
		}
		 ...
		 */
		load_theme_textdomain( $this->namespace, WMS_REST_API_DIRNAME . '/languages' );
	...
	function admin_options_page() {
		...
		include( WMS_REST_API_DIRNAME . "/views/view-options.php" );
	}
	...
	static function instance() {
		...
		if ( ! isset( $Wms_Rest_Api ) ) {
			$Wms_Rest_Api = new Wms_Rest_API();
		}
	}
	...
	function plugin_action_links( $links, $file ) {
		if ( $file == plugin_basename( WMS_REST_API_DIRNAME . '/' . basename( __FILE__ ) ) ) {
			$old_links = $links;
			$new_links = array(
	 ...
	 * form processor.
	 *
	 * @uses Wms_Rest_API::_admin_options_update()
	 */
	function route() {
	...
	function wp_register_scripts() {
		// Admin JavaScript
		wp_register_script( "{$this->namespace}-admin", WMS_REST_API_URLPATH . "/js/admin.js", array( 'jquery' ), $this->version, true );
	}
	...
	function wp_register_styles() {
		// Admin Stylesheet
		wp_register_style( "{$this->namespace}-admin", WMS_REST_API_URLPATH . "/css/admin.css", array(), $this->version, 'screen' );
	}
}

if ( ! isset( $Wms_Rest_Api ) ) {
	Wms_Rest_API::instance();
}

register_activation_hook( __FILE__, array( 'Wms_Rest_API', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'Wms_Rest_API', 'deactivate' ) );
